#!/usr/bin/env python3
"""Render the committed support workflow with the operator-approval continuation gate."""
...
def code_node(node_id: str, name: str, position: list[int], js: str) -> dict[str, Any]:
    return {
...
def write_node(node_id: str, name: str, position: list[int], prop: str, path: str) -> dict[str, Any]:
    return {
        "parameters": {
            "operation": "write",
            "fileName": path,
...
def connection(target: str) -> dict[str, Any]:
    return {"node": target, "type": "main", "index": 0}


def replace_node(nodes: list[dict[str, Any]], name: str) -> dict[str, Any]:
...
def render(source: dict[str, Any]) -> dict[str, Any]:
    ...
    draft = replace_node(nodes, "Draft Reply Through Talon")
    draft["parameters"]["jsonBody"] = (
        "={{ { model: 'gpt-4o-mini', max_tokens: 350, messages: [{ role: 'user', "
        f"content: {json.dumps(prompt)} + $json.case_package }}] }} }}"
    )

    require_reply = replace_node(nodes, "Require Reply Draft")
    require_reply["parameters"]["jsCode"] = """const source = $('Assemble Support Case').item.json;
...
return { json: { ...source, reply_body: body, requested_model: 'gpt-4o-mini', provider_reported_model: String($json.body?.model || 'not-recorded') } };"""
    ...
    approval_nodes = [
        ...
        code_node(
            "approval-pending-0001",
            "Require Pending Approval",
            [200, 120],
            """const source = $('Require Refund Action Denial').item.json;
const status = Number($json.statusCode || 0);
...
return { json: { ...source, approval_id: request.approval_id, approval_url: String($json.body.approval_url || '') } };""",
        ...
        code_node(
            "approval-validate-0001",
            "Validate Operator Decision",
            [600, 120],
            """const source = $('Require Pending Approval').item.json;
const status = Number($json.statusCode || 0);
...
return { json: { ...source, operator_decision: receipt.decision, operator_approval_receipt: receipt, human_approval_completed: true } };""",
    ...
    for value in connections.values():
        for branch in value.get("main", []):
            branch[:] = [edge for edge in branch if edge.get("node") not in REMOVE]
    connections["Require Refund Action Denial"] = {"main": [[connection("Create Human Approval Request")]]}
    connections["Create Human Approval Request"] = {"main": [[connection("Require Pending Approval")]]}
    connections["Require Pending Approval"] = {"main": [[connection("Wait for Operator Decision")]]}
    connections["Wait for Operator Decision"] = {"main": [[connection("Validate Operator Decision")]]}
    connections["Validate Operator Decision"] = {"main": [[connection("Operator Approved?")]]}
    connections["Operator Approved?"] = {
        "main": [[connection("Build Approved Artifacts")], [connection("Build Rejected Artifacts")]]
    }
    connections["Build Approved Artifacts"] = {"main": [[connection("Write Approved Receipt")]]}
    connections["Write Approved Receipt"] = {"main": [[connection("Write Finance Handoff")]]}
    connections["Write Finance Handoff"] = {"main": [[connection("Write Approved Resolution")]]}
    connections["Write Approved Resolution"] = {"main": [[connection("Write Approved Status")]]}
    connections["Build Rejected Artifacts"] = {"main": [[connection("Write Rejected Receipt")]]}
    connections["Write Rejected Receipt"] = {"main": [[connection("Write Rejected Resolution")]]}
    connections["Write Rejected Resolution"] = {"main": [[connection("Write Rejected Status")]]}
...
REJECTED_JS = """const source = $json;
const receipt = source.operator_approval_receipt;
const status = { status: 'completed', result: 'governed_support_resolution_rejected', session_id: receipt.session_id, run_nonce: receipt.run_nonce, operational_id: 'customer-support', documents_read: source.document_count, ticket_id: source.ticket_id, refund_amount_eur: source.refund_amount_eur, reply_draft_created: true, blocked_tool: source.blocked_tool, denial_code: source.denial_code, denied_provider_cost_usd: 0, action_status: 'human_rejected', operator_approval_required: true, human_approval_completed: true, operator_decision: 'rejected', approval_id: source.approval_id, requested_model: source.requested_model, provider_reported_model: source.provider_reported_model, pii_expected: ['email', 'iban'], report_file: 'customer-support-resolution.md', approval_receipt_file: 'operator-approval-receipt.json', finance_handoff_file: null, refund_executed: false };
