...
for cmd in node npm unzip sha256sum find; do need "$cmd"; done
...
say "Validating Zendesk app with ZCLI $ZCLI_VERSION..."
run_zcli apps:validate "$APP"
say 'Packaging Zendesk private app...'
run_zcli apps:package "$APP"
...
for forbidden in '.env' 'zcli.apps.config.json' 'node_modules/' '/tmp/' 'adapter_token' 'TALON_.*KEY'; do
